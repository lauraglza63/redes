...
class EntityA:
    ...
    def output(self, message):
        ...
        self.handle_event(self.exit)

    def input(self, packet):
        self.handle_event(self.entry, packet)

    def timer_interrupt(self):
        self.handle_event(self.timer)

    def handle_event_wait_for_call(self, event, arg=None):
        if event == self.exit:
            if not self.msg:
                return
            packet = Pkt(self.bit, 0, 0, self.msg.pop(0).data)
            add_checksum(packet)
            to_layer3(self, packet)
            self.sent_pckt = packet
            start_timer(self, self.wait_time)
            self.handle_event = self.handle_event_wait_for_ack

        elif event == self.entry:
            pass

        elif event == self.timer:
            if TRACE > 0:
                print('Entity A: Unexpected timeout')

        else:
            print(f'Entity A: Unknown event {event}.')

    def handle_event_wait_for_ack(self, event, arg=None):
        if event == self.exit:
            pass

        elif event == self.entry:
            pckt = arg
            if (checksum_for_pckt(pckt) != pckt.checksum or pckt.acknum != self.bit):
                return
            stop_timer(self)
            self.bit = 1 - self.bit
            self.handle_event = self.handle_event_wait_for_call
            self.handle_event(self.exit)
            self.handle_event_wait_for_call(self.exit)

        elif event == self.timer:
            to_layer3(self, self.sent_pckt)
            start_timer(self, self.wait_time)
...
class EntityB:
    ...
    def input(self, packet):
        if (checksum_for_pckt(packet) != packet.checksum or packet.seqnum != self.expecting_bit):
            pckt = Pkt(0, 1-self.expecting_bit, 0, packet.payload)
            add_checksum(pckt)
            to_layer3(self, pckt)
        else:
            to_layer5(self, Msg(packet.payload))
            pckt = Pkt(0, self.expecting_bit, 0, packet.payload)
            add_checksum(pckt)
            to_layer3(self, pckt)
            self.expecting_bit = 1 - self.expecting_bit

    def timer_interrupt(self):
        print("Entity B: Timer interrupt")


def checksum_for_pckt(packet):
    checksum = 0
    checksum = crc32(packet.seqnum.to_bytes(4, byteorder='big'), checksum)
    checksum = crc32(packet.acknum.to_bytes(4, byteorder='big'), checksum)
    checksum = crc32(packet.payload, checksum)
    return checksum


def add_checksum(packet):
    packet.checksum = checksum_for_pckt(packet)
